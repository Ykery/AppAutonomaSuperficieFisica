...
QComboBox *FFTSamples;  
QwtThermo *lockinSignal1; 
QTime theTime; 

TaskHandle lockinTskHdl;  #-> variable de tipo 'TaskHandle', para uso de la interfaz NI (IN/OUT supongo)
double DACParams[7]; 
...
configScan::configScan(QWidget *parent)
     ...
     grid->addWidget(createTeasDACBox(), 0, 0, 3, 2); 
     
     grid->addWidget(createScanAMLGaugeBox(), 0, 2, 2, 2); 
     grid->addWidget(createTeasSamplingBox(), 2, 2, 1, 1); 
     grid->addWidget(createTeasChanneltronBox(), 2, 3, 1, 1); 
     
     grid->addWidget(createlockinThermo(), 3, 0, 1, 2); 
     grid->addWidget(teasSystemIDBox(), 3, 2, 1, 2); 
     
     grid->addWidget(setDataFile(), 4, 0, 1, 3); 
	 
     grid->addLayout(buttonsLayout, 5, 3, 1, 1); 
...
QGroupBox *configScan::createTeasSamplingBox()
     {
     teasSampleBox = new QGroupBox(tr("Sampling parameters"));
     teasSampleBox->setFont(QFont("Helvetica", 11));     
     teasSampleBox->setCheckable(true); 
     teasSampleBox->setChecked(false); 
   
     FFTSamples = new QComboBox; 
     FFTSamples->setGeometry(QRect(10, 50, 200, 50));
     FFTSamples->insertItems(0, FFTSamplesVals); 

     QLabel *sampleLabel = new QLabel(tr("FFT TEAS samples")); 

     sampleCheckBox = new QCheckBox(tr("Check when correct values set"));
     sampleCheckBox->setFont(QFont("Helvetica", 10.5));   
     sampleCheckBox->setChecked(false);

     connect(sampleCheckBox, SIGNAL(stateChanged(int)), this, SLOT(disableFFTSamplesCombo(int))); 

     QVBoxLayout *layout1 = new QVBoxLayout;
     layout1->addWidget(sampleLabel); 
     layout1->addWidget(FFTSamples);
     layout1->addWidget(sampleCheckBox);
     teasSampleBox->setLayout(layout1);

     return teasSampleBox;
     }
...
void configScan::setTimeScanParameters()
     ...
     switch (k = FFTSamples->currentIndex()) 
          { 
          case 1: 
               parametersList[0] = 131072; 
               break; 
          case 2: 
               parametersList[0] = 65536; 
               break; 
          case 3: 
               parametersList[0] = 32768; 
               break; 
          case 4: 
               parametersList[0] = 16384; 
               break; 
          case 5: 
               parametersList[0] = 8192; 
               break; 
          } 
     printf ("%lf\n", parametersList[0]); 
...
void configScan::disableFFTSamplesCombo(int c)
     { 
     if ( c ) 
          FFTSamples->setEnabled(false); 
     else if ( teasSampleBox->isChecked() && !sampleCheckBox->isChecked() )
          FFTSamples->setEnabled(true); 
     } 
...
void configScan::runTimeScan() 
     ...
     else if ( !sampleCheckBox->isChecked() ) 
          {         
          msgBox->setText(tr("Please set sampling parameters and check box")); 
          msgBox->exec(); 
          return; 
          } 
     else if ( !FFTSamples->currentIndex() ) 
          { 
          msgBox->setText(tr("Please set number of samples for TEAS measurement")); 
          msgBox->exec(); 
          return; 
          } 
...
void configScan::updateSettings() 
	...
	h.sprintf( "%02d:%02d", thisTime.hour(), thisTime.minute()); 
     ...
     indicesList[3] = FFTSamples->currentIndex(); 
     indicesList[4] = scanAMLUnitsComboBox->currentIndex(); 

     settingsList << dataFileName; 
     settingsList << dd; 
     settingsList << h; 
     settingsList << FFTSamples->currentText(); 
