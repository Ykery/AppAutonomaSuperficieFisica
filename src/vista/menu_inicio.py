...
class MenuInicio(QWidget):
    def __init__(
        ...
        btn_teas.clicked.connect(lambda: self.open_child_window(TeasWindow))
        btn_moke.clicked.connect(
            lambda: self.open_child_window(MokeWindow)
        )
        btn_experimentos.clicked.connect(
