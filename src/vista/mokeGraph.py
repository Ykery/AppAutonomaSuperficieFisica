...
import numpy as np
from src.modelo.dao import ResultadoMokeDAO, ExperimentoDAO
...
class MokeGraph( QWidget ):
    def __init__(self, id, load_results = False, *args):
        ...
        if self.load_results:
            ...
            for resultado in resultados_cargados:
                self.datax.append(resultado.magnetic_field)
        ...
        self.d_plot =  Plot("Moke Loop Graph", "Magnetic field (Oe)", "Intensity [arb. un.]", self, color=QColor(131, 25, 70) )
    ...
    def actualizarDatos(self):
        if not self.load_results and not self.finished:
            ...
            resultado.magnetic_field = self.datax[-1]
            resultado.intensity = self.datay[-1]
            escribir_csv(self.experimento.rutaCsv, resultado.magnetic_field, resultado.intensity)
